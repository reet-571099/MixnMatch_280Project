...
import { getSavedRecipe, type SavedRecipe } from "@/lib/authClient";
...
const RecipeDetailPage = () => {
  ...
  // Check if this is a saved recipe route
  const isSavedRecipe = location.pathname.startsWith('/saved-recipes/');
  ...
  useEffect(() => {
    const loadRecipe = async () => {
      try {
        ...
        if (isSavedRecipe) {
          // Load saved recipe from MongoDB
          ...
          const savedRecipe = await getSavedRecipe(token, id || '');
          if (savedRecipe) {
            // Convert SavedRecipe to Recipe format
            const convertedRecipe: Recipe = {
              id: parseInt(savedRecipe.id),
              ...
              tags: [savedRecipe.cuisine, savedRecipe.mealType, savedRecipe.difficulty],
              ...
              imageUrl: '', // No image for saved recipes
              author: {
                name: "Your Saved Recipe",
                isAI: false
              },
              rating: 0,
              ratingCount: 0,
            ...
            setError("Saved recipe not found");
          }
        } else {
          // Load recipe from JSON (original logic)
      ...
      <main className="min-h-screen bg-gradient-to-br from-background via-background to-secondary/20 flex items-center justify-center">
        ...
        >
          ...
          <Button asChild className="mt-6">
            <Link to="/favorites" className="inline-flex items-center gap-2">
        ...
        <div className="container mx-auto px-4 py-6 max-w-6xl">
          ...
          >
            <Button 
              ...
              aria-label={isSavedRecipe ? "Go back to Favorites" : "Go back to Explore recipes"}
            >
              <Link to={isSavedRecipe ? "/favorites" : "/explore"}>
                <ArrowLeft className="h-4 w-4" />
                {isSavedRecipe ? "Back to Favorites" : "Back to Explore"}
              </Link>
            </Button>
          </motion.nav>

          {isSavedRecipe ? (
            // Simple layout for saved recipes
            <div className="max-w-4xl mx-auto">
              ...
              >
                ...
                <Card className="p-4 text-center">
                  <div className="h-6 w-6 mx-auto mb-2 text-primary flex items-center justify-center">🍽️</div>
                  <p className="text-sm text-muted-foreground">Cuisine</p>
                  <p className="font-semibold capitalize">{recipe.cuisine}</p>
                </Card>
              </motion.div>
                    ...
                    <div className="space-y-4">
                      {recipe.steps.map((step) => (
                        <motion.div
                          ...
                          animate={{ opacity: 1, y: 0 }}
                          transition={{ delay: 0.3 + step.stepNumber * 0.1 }}
                          className="flex gap-4 p-3 rounded-lg hover:bg-muted/50 transition-colors"
                        >
                          <div className="w-8 h-8 rounded-full bg-primary text-primary-foreground font-semibold flex items-center justify-center flex-shrink-0">
                            {step.stepNumber}
                          </div>
            ...
            // Original complex layout for JSON recipes
            <>
              {/* Top Chip Row */}
              <motion.header
                variants={fadeInUp}
